...
type ProductForApiInput = Prisma.ProductGetPayload<{ include: typeof productInclude }>;
export function productForApi(product: ProductForApiInput, includeCostPrice = false) { const { costPrice, salePrice, ...rest } = product; return { ...rest, salePrice: Number(salePrice), ...(includeCostPrice ? { costPrice: Number(costPrice) } : {}) }; }
