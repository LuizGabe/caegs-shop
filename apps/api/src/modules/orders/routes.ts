...
export const orderRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch("/admin/orders/:publicId/status", { preHandler: requireAdmin }, async (request, reply) => { const publicId = publicIdSchema.parse(request.params).publicId; const input = statusUpdateSchema.parse(request.body); const order = await prisma.order.findUnique({ where: { publicId }, include: orderInclude }); if (!order) return notFound(reply); const updated = await prisma.$transaction(async (tx) => { const next = await tx.order.update({ where: { id: order.id }, data: { ...(input.paymentStatus ? { paymentStatus: input.paymentStatus } : {}), ...(input.fulfillmentStatus ? { fulfillmentStatus: input.fulfillmentStatus } : {}) }, include: orderInclude }); await tx.orderStatusHistory.create({ data: { orderId: order.id, previousPaymentStatus: order.paymentStatus, newPaymentStatus: input.paymentStatus ?? order.paymentStatus, previousFulfillmentStatus: order.fulfillmentStatus, newFulfillmentStatus: input.fulfillmentStatus ?? order.fulfillmentStatus, changedByUserId: request.currentUser!.id, source: "ADMIN", note: input.note ?? null } }); return next; }); return { order: orderForApi(updated) }; });
};