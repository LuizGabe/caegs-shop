...
describe("product catalogue administration", () => {
  ...
  it("creates, edits, manages variants and images, and soft deletes products", async () => {
    ...
    const inactive = await app.inject({ method: "PATCH", url: `/admin/products/${product.id}`, headers, payload: { active: false } }); expect(inactive.statusCode).toBe(200); expect((await app.inject({ method: "GET", url: `/products/${product.slug}` })).statusCode).toBe(404);
