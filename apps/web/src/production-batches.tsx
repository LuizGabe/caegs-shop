...
import { ArrowRight, CheckSquare, PackageCheck, Plus } from "lucide-react";
...
type BatchOrder = { id: string; publicId: string; user: { name: string; email: string }; total: number; items: Array<{ id: string; productName: string; variantName: string; quantity: number }> };
...
function BatchDetail({ batchId }: { batchId: string }) {
  ...
  return <div><header className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5"><div><p className="text-sm text-muted-foreground">{batch.code}</p><h2 className="mt-1 text-2xl font-semibold">{batch.name}</h2><span className="mt-2 inline-block rounded bg-muted px-2 py-1 text-xs font-medium">{statusLabel[batch.status]}</span></div>{next && <button disabled={transition.isPending} onClick={() => transition.mutate(next)} className="inline-flex items-center gap-2 rounded bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-60">{statusLabel[next]}<ArrowRight size={17} /></button>}</header>{transition.error && <p className="mt-3 text-sm text-red-700">{transition.error.message}</p>}{batch.notes && <p className="mt-5 whitespace-pre-line text-sm text-muted-foreground">{batch.notes}</p>}{batch.status === "DRAFT" && <OrderSelector batchId={batchId} />}{batch.summary.length > 0 && <BatchSummary summary={batch.summary} />}<div className="mt-8"><h3 className="font-semibold">Pedidos no lote <span className="text-muted-foreground">({batch.orders.length})</span></h3><div className="mt-3 divide-y border-y border-border">{batch.orders.map((order) => <div key={order.id} className="grid gap-2 py-3 sm:grid-cols-[1fr_auto]"><div><p className="font-medium">{order.user.name}</p><p className="text-xs text-muted-foreground">{order.publicId} · {order.items.map((item) => `${item.productName} ${item.variantName} x${item.quantity}`).join(", ")}</p></div><span className="text-sm font-medium">{currency.format(order.total)}</span></div>)}</div></div><div className="mt-8"><h3 className="font-semibold">Historico</h3><div className="mt-3 divide-y border-y border-border">{batch.auditLogs.map((log) => <div key={log.id} className="flex justify-between gap-4 py-3 text-sm"><span>{auditLabel(log.action)}</span><time className="shrink-0 text-muted-foreground">{new Date(log.createdAt).toLocaleString("pt-BR")}</time></div>)}</div></div></div>;
}
